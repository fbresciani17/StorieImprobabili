...
export default function EditorScreen() {
  ...
  // Se arriviamo da "Modifica", troviamo story nei params
  const editingStory = route.params?.story ?? null;

  const [title, setTitle] = useState('');
  const [body, setBody] = useState('');

  // Quando cambia la storia in modifica, precompila i campi
  useEffect(() => {
    if (editingStory) {
  ...
  async function handleSave() {
    ...
    try {
      ...
      // reset campi e modalità
      setTitle('');
      setBody('');
      // rimuovi i params di modifica, così torni in modalità "nuova"
